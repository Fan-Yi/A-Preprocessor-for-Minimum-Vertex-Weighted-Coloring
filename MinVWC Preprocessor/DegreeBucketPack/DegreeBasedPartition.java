...
public class DegreeBasedPartition{
	...
	public DegreeBasedPartition(int max_degree, int vertex_num, ArrayList<Integer> remaining_vertex_list, Vertex[] vertices)
	{
		...
		for(int d = 1; d <= dynamic_max_degree + 1; d++)// initialize enough locations to avoid NullPointException
		{
			...
			ptr_to_out_vertices.add(ptr_to_in_vertices.get(d));
			qtr_to_v_degree.add(ptr_to_in_vertices.get(d));
		}
		ptr_to_out_vertices.add(dynamic_max_degree + 1);
	...
	public ArrayList<Integer> get_vertex_list_of_top_degree(int k)
	{
	...
	public boolean check_partitions(int vertex_num, Vertex[] vertices, ArrayList<Integer> remaining_vertex_list, ArrayList<Integer> removed_vertex_list)
	{
