...
public class SimpleUndirectedVertexWeightedGraphDIMACSParser{
	...
	{
				if(con_s_in.findInLine("p edge ") == null && con_s_in.findInLine("p Edge ") == null && con_s_in.findInLine("p col ") == null)
				{
				...
				if(con_s_in.hasNextInt())
				{
					...
					if(con_s_in.hasNextInt())
					{
						...
						{
							System.out.println("Format error occurs when reading the head line: the 4th token is invalid.");
							return false;
						}
					}
					else
	...
	public static boolean deal_with_vertex_line(Scanner con_s_in, int[] a_int)
	{
				...
				{
					System.out.println("Format error occurs when reading the head line: the first two chars are invalid.");
					return false;
				}
