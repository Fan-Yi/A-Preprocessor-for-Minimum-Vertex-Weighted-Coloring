...
public class WeightBasedPartition{
	...
	public WeightBasedPartition(int max_weight, int vertex_num, ArrayList<Integer> remaining_vertex_list, WeightedVertex[] vertices)
	{
	...
	public ArrayList<Integer> get_vertex_list_of_top_weight(int k)
	{
		...
		int min_weight_in_loop = StrictMath.max(dynamic_max_weight - k + 1, 0);
	...
	{
		ArrayList<Integer> ret_list = new ArrayList<Integer>();
		int min_w_in_loop = StrictMath.max(min_w, 0);
	...
	public boolean check_partitions(int vertex_num, WeightedVertex[] vertices, ArrayList<Integer> remaining_vertex_list, ArrayList<Integer> removed_vertex_list)
	{
		for(int i = 0; i < remaining_vertex_list.size(); i++)
		{
		...
		for(int w = 0; w <= dynamic_max_weight; w++)
		{
			for(int i = ptr_to_in_vertices.get(w); i < ptr_to_out_vertices.get(w); i++)
			{
				int v = v_ascending_weight_array.get(i);
				if(!remaining_vertex_list.contains(v))
				{
					System.out.println(v + " should be in the remaining_vertex_list according to partitions, conflicts between partitions and dynamic vertex sets");
					return false;
				}
			...
			for(int i = ptr_to_out_vertices.get(w); i < ptr_to_in_vertices.get(w+1); i++)
			{
				...
				if(remaining_vertex_list.contains(v))
				{
					System.out.println(v + " should not be in the remaining_vertex_list according to partitions, conflicts between partitions and dynamic vertex sets");
					return false;
				}
				if(!removed_vertex_list.contains(v))
				{
